...
const updateDateTime = () => {
  ...
  dateDisplay.innerHTML = `${currentDay}, ${ordinalNumbers(
    currentDate
  )} of ${currentMonth} ${currentYear}`;
};
...
// Fetch requests
// Unsplash API
fetch(`https://api.unsplash.com/photos/random?query=naturedark&client_id=${unsplashAPIKey}`)
  .then((res) => res.json())
  .then((image) => {
  ...
  .then((eth) => {
    cryptoContainer.innerHTML = `
      ...
      <div class="crypto-info">
        ...
        <li>Current price: <span>$${eth.market_data.high_24h.usd.toFixed(2)}</span></li>
        <li>Highest price: <span>$${eth.market_data.current_price.usd.toFixed(2)}</span></li>
        <li>Lowest price: <span>$${eth.market_data.low_24h.usd.toFixed(2)}</span></li>
        </ul>
      </div>
  `;
  });
...
navigator.geolocation.getCurrentPosition((position) => {
  fetch(
    ...
    .then((res) => {
      if (!res.ok) throw Error("Weather data is not availabe");
      return res.json();
    })
    .then((weather) => {
